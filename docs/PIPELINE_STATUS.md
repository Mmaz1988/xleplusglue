...
| `LOGGING_HYGIENE_PLAN.md` | Implemented 2026-08-11 in all three services (`inference/logging_config.py` + `LOG_LEVEL`/`LOG_DIR`; liger and GSWB logback configs behind `LIGER_LOG_LEVEL`/`GSWB_LOG_LEVEL`; both mains now forward `--logging.file.name`). Jars **not** rebuilt, so `jars/liger.jar`/`jars/gswb.jar` still carry the old logging; the live line-count/discourse checks (plan's verification 1-4) are outstanding — see the plan's "What was actually done" section |
...
| `liger` | `docs/bug_reports/liger_replace_flag_value_variables_not_substituted.md` | Fixed 2026-08-13 (`QueryParser.parseQuery` was filtering `valueBindings` down to exact-match-only, dropping any `%`-binding recorded before a later conjunct grew the Solution — root-caused to commit `21171d2`). Regression tests added to `RuleParserTest`/`QueryParserTest`; `jars/liger.jar` rebuilt and copied. Also fixed same session: an unbound `?=>` right-hand-side node/value variable now shares one introduced node across all of a rule's conjuncts instead of minting a fresh one per occurrence, and rule solutions whose bound right-hand-side output is identical are conflated into one annotation/branch instead of firing once per redundant left-hand-side solution |
